...
# Install the release package without enabling the broken auto-generated repo first.
...
# The release package points SP3 systems to a non-existent Antelope repo.
if [ -f "${ANTELOPE_REPO_FILE}" ]; then
  sed -ri 's#openEuler-24.03-LTS-SP3#openEuler-24.03-LTS-SP2#g' "${ANTELOPE_REPO_FILE}"
fi

# The default metalink endpoints can be unstable in this lab; prefer direct baseurl.
...
# After power loss or a reset, the controller clock can drift badly enough to
# make OpenStack heartbeats look stale. Force an initial step if chronyd allows
# it, then print tracking data for verification.
