...
1. Construct both upload and staging paths as `/opt/.openstack_repo.<zip-sha256>.<random-UUID-or-task-nonce>` with distinct suffixes for the archive and directory.
2. Record `lstat` and `test ! -e` results immediately before creation.
3. Create the staging directory with an exclusive `mkdir`; an existing path is a hard stop.
4. Upload and extract only into those nonce-bearing paths.
5. Compare the full normalized entry set, type of every entry, size of every regular file, `repomd.xml` SHA256, and the eight core RPM SHA256 values before atomic promotion.
6. Remove only the exact task-owned upload file and empty staging directory after promotion, then record their absence.
...
The report path was parsed and constrained to a single basename directly below `/root/openstack-repo-backups/`. Before deletion, `/root/openstack-repo-backups/vsftpd.conf.pre-openstack-repo-ddae4b9bd63b20e5ebb052e52d86afb8a98f6edfeefde799f8ccf966aae65978` was verified as a 5,039-byte regular file with SHA256 `fa52b7c07f7499930bbd5fdd890b3828d55250bfa39361837944f6236408e0f6`. That hash exactly matched `etc/vsftpd/vsftpd.conf` streamed from the offline vsftpd RPM, confirming it was the package default captured after installation and before this task's configuration replacement.
