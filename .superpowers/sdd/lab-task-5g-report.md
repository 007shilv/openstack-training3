...
- 后续本地复审修正了教材的防护表达：根盘祖先链先检查命令返回值和非空性，再逐项规范化比较；磁盘先精确读取 PV，只有无 PV 时才接受零签名和 `blkid` 返回 2。唯一 `/dev/sdb`→`cinder-volumes` PV 是幂等状态，但仍须只读验证签名集合恰为 `LVM2_member`；空签名、第二种签名或探针失败均拒绝，且不会运行 `pvcreate`/`vgcreate`。孤立、外来或重复 PV 同样拒绝。本次修订没有连接或变更虚拟机。
